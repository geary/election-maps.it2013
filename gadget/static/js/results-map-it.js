...
// Default params
var defaultElectionKey = '2013-senate-1';
...
//		stateUS;
//	state.electionTitle = S( state.name, ' ', state.type || T('primary') );
...
//opt.zoom = opt.zoom || 3;
opt.fontsize = '15px';
var sidebarWidth = 290;  // params.play ? 340 : 280;
...
document.write(
	'<style type="text/css">',
		...
		'#maptip { position:absolute; z-index:10; border:1px solid #333; background:white; color:#222; white-space: nowrap; display:none; min-width:260px; }',
		...
		//'#anp-logo { right:64px; width:48px; background: url(', imgUrl('anp-tk2013-48x48-masked.png'), ') no-repeat; display:none; }',
...
//	'</a>',
	'<a id="google-logo" class="logo" target="_blank" href="http://www.google.it/elections/ed/it/home?hl=', params.hl, '" title="', T('googlePoliticsTitle'), '">',
	'</a>',
		...
		return S(
			'<div id="buttons" style="position:relative;">',
				...
				//	//'</a>',
					...
					'</a>',
					'&nbsp;',
					'<a class="button', params.contest == 'chamber' ? ' selected' : '', '" id="btnContest-chamber">',
						T('chamber'),
					'</a>',
				//	//'&nbsp;&nbsp;&nbsp;&nbsp;',
...
(function( $ ) {
	...
	loadGeoJSON = function( json ) {
		...
		var geoid = json.region ? json.region.id : json.deputy.id;
		current.national = ( geoid == 'IT' );
		current.geoid = geoid;
		if( ! geoJSON[geoid] ) {
			geoJSON[geoid] = json;
...
*/
	...
	function colorize() {
		if( params.province ) {
			colorVotes( provincialGeo(), '#444444', 1, 1 );
			colorSimple( regionalGeo(), '#FFFFFF', '#444444', 1, 2 );
		}
		else {
			colorVotes( regionalGeo(), '#222222', 1, 2 );
		}
		...
		function formatRow( iCount, title ) {
			...
			return count ? S(
				'<tr>',
					...
					'<td>', T(title), ' (', formatPercent( count / total ),  ')</td>',
					//'<td>', T(title), '</td>',
	...
	function formatSidebar() {
		...
		if( results ) {
			...
			var viewNational = nationalEnabled() ? S(
				'<a href="#" id="viewNational" title="', T('titleViewNational-fr'), '" style="">',
					T('viewNational-fr'),
				'</a>'
			) : '&nbsp;';
			resultsHeaderHTML = S(
		...
		//	params.embed_state
		//) ? S(
		//	'<a href="http://www.google.com/elections/ed/us/results/2013/gop-primary/',
	...
	function formatSidebarTopCandidates( topCandidates ) {
		...
		return S(
			'<tr class="legend-candidate', selected, '" id="legend-candidate-top" title="', T('clickForAllCoalitionsMap'), '">',
				'<td class="left">',
	...
	function formatSidebarCandidate( candidate ) {
		var selected = ( candidate.id == current.coalition ) ? ' selected' : '';
		return S(
	...
	function formatCandidateList( topCandidates, formatter, header ) {
		...
		var thead = header ? S(
			'<tr>',
				...
				//	//current.national  &&  view != 'county' ? T('delegatesAbbr') : '',
				//'</th>',
			'</tr>'
		) : '';
		return S(
	...
	function formatTip( where ) {
		...
		//			T( feature.click === false ? 'noLocal' : touch ? 'tapForLocal' : 'clickForLocal' ),
		//		'</div>'
		//	) : '';
		// TODO
	...
	function formatNumber( nStr ) {
		var dsep = T('decimalSep'),
			tsep = ( T('thousandsSep') || ' ' ).replace( ' ', '&nbsp' );
		nStr += '';
		x = nStr.split('.');
		x1 = x[0];
...
//		var $topbar = $('#topbar');
		...
		$sidebar.delegate( '#btn2010,#btn2013', {
			click: function( event ) {
	...
	function addRegionalResults( json ) {
		...
		addCoalitionResults( json.table );
		addCoalitionResults( json.regional );
	}
	
	function addCoalitionResults( table ) {
		var colCoa = makeCols( election.coalitions );
		var col = table.cols, colID = _.indexOf( col, 'ID' );
		
		var parties = election.parties, coalitions = election.coalitions;
		
		var rowsCoa = [];
		
		function newRow( id ) {
			row = [];
			for( var iCol = -1;  ++iCol < colCoa.length; )
				row[iCol] = 0;
			row[colCoa.ID] = id;
			return row;
		}
		
		var rowsCoa = _.map( table.rows, function( row ) {
			var rowCoa = newRow( row[colID] );
			
			for( var iCol = -1;  ++iCol < colID; ) {
				var idParty = col[iCol].replace( 'TabCount-', '' );
				var party = parties.by.id[idParty];
				var coalition = party.coalition;
				var iColCoa = coalition.index;
				rowCoa[iColCoa] += row[iCol];
			}
			
			rowCoa[colCoa.ID] = row[colID];
			
			for( var iCol = colID;  ++iCol < col.length; ) {
				rowCoa[ colCoa.ID - colID + iCol ] = row[iCol];
			}
	...
	function fixupResults( json, results, geo ) {
		...
		fixupResultsOne( json, results, geo );
		fixupResultsOne( json, results.parties, geo );
	}
	
	function fixupResultsOne( json, results, geo ) {
		...
		election.coalitions.forEach( function( coalition ) {
