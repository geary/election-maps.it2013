...
indexArray( candidatesCZ2013, 'id' );
...
//	'</a>',
	'<a id="google-logo" class="logo" target="_blank" href="http://www.google.cz/elections/ed/cz" title="', T('googlePoliticsTitle'), '">',
	'</a>',
...
(function( $ ) {
	...
	loadGeoJSON = function( json ) {
		...
		var geoid = json.district.id;
		current.national = ( geoid == 'CZ' );
		current.geoid = geoid;
		if( ! geoJSON[geoid] ) {
			geoJSON[geoid] = json;
...
//		features.by['GM0998'] = feature;
	...
	function currentGeos() {
		...
		return [ json.district, json.region, json.nation ];
	}
...
*/
	...
	function colorize() {
		var json = geoJSON[current.geoid];
		colorVotes( json.district, '#666666', 1, 1 );
		colorSimple( json.region, '#FFFFFF', '#444444', 1, 2 );
