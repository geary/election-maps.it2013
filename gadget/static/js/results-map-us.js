...
document.write(
	'<style type="text/css">',
		...
		'a.button.selected { background-color: #EEE; background-image:linear-gradient(top,#EEE,#E0E0E0); border:1px solid #CCC; box-shadow:inset 0px 1px 2px rgba(0,0,0,0.1); color:#333; z-index: 2; }',
                'a.segmented.button { border-radius: 0; text-align: center; padding: 0 8px; width: 50px; }',
...
*/
	...
	function insetFeatures( abbr, callback ) {
		/*if( view == 'county' ) {
			var cf = stateUS.geo.county.features.by;
		}
		else*/ if( params.contest != 'house' ) {
			var sf = stateUS.geo.state.features.by;
		}
		else {
			var hf = stateUS.geo.house.features.by;
		}
		if( abbr == 'AK' ) {
			sf && callback( sf.AK || sf['02'] );
			//cf && callback( cf['02'] );
			hf && callback( hf['AK-AL'] );
		}
		else {  // HI
			sf && callback( sf.HI || sf['15'] );
			...
			'<tr class="legend-candidate', selected, zero, '" id="legend-candidate-', candidate.id, '" title="', T('clickForHeatMap'), '">',
				...
				'<td', !isPres ? ' class="right"' : '', '>',
					'<div class="legend-candidate" style="text-align:right;">',
						...
						'</div>',
						'<div class="legend-candidate-secondary">',
